  ...
  while (catIds.size < numCats) {
    try {
      let response = await axios.get("https://jservice.io/api/category", {
        params: { id: Math.floor(Math.random() * 20000) },
      });
...
async function fillTable() {
  for (let i = 0; i < catIds.length; i++) {
    ...
    $("#row1").append(
      `<td id='${i + 1}-1' data-catId='${catIds[i]}' data-column='${
        i + 1
      }' data-row='1'>?</td>`
    );
    $("#row2").append(
      `<td id='${i + 1}-2' data-catId='${catIds[i]}' data-column='${
        i + 1
      }' data-row='2'>?</td>`
    );
    $("#row3").append(
      `<td id='${i + 1}-3' data-catId='${catIds[i]}' data-column='${
        i + 1
      }' data-row='3'>?</td>`
    );
    $("#row4").append(
      `<td id='${i + 1}-4' data-catId='${catIds[i]}' data-column='${
        i + 1
      }' data-row='4'>?</td>`
    );
    $("#row5").append(
      `<td id='${i + 1}-5' data-catId='${catIds[i]}' data-column='${
        i + 1
      }' data-row='5'>?</td>`
    );
    let selectedClues = [];
    let questions = new Set();
    if (catData.clues.length < 5) {
      console.log("error - not enough clues");
    } else {
      while (selectedClues.length < 5) {
        let clue =
          catData.clues[Math.floor(Math.random() * catData.clues.length)];
...
async function setupAndStart() {
  ...
  $("#content").append(
    '<table id="jeopardy-table"><thead><tr id="cat-row"></tr></thead><tbody><tr id="row1">100</tr><tr id="row2">200</tr><tr id="row3">300</tr><tr id="row4">400</tr><tr id="row5">500</tr></tbody></table>'
  );
